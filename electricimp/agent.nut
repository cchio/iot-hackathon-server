...
/******************** APPLICATION CODE ********************/
const PUBKEY = "pub-c-ee09f7e5-2bdf-4573-ac06-6148cf772de6";
const SUBKEY = "sub-c-0672ac1e-35f1-11e4-afa1-02ee2ddab7fe";
const SECRETKEY = "sec-c-N2E2Mzc5ZmEtMzc4ZS00MGVhLThmNDMtZDJkMmI0ZmQ0NGRk";
...
function rawDataToSignal(accelData) {
    ...
    if (accelData.len() == 3) {
        if (accelData.x > ACCEL_THRESHOLD
            && math.abs(accelData.y) < ACCEL_THRESHOLD
            && math.abs(accelData.z) < ACCEL_THRESHOLD) {
            signal = 0;
        } else if (accelData.x < -ACCEL_THRESHOLD
            && math.abs(accelData.y) < ACCEL_THRESHOLD
            && math.abs(accelData.z) < ACCEL_THRESHOLD) {
            signal = 1;
        } else if (math.abs(accelData.x) < ACCEL_THRESHOLD
            && accelData.y > ACCEL_THRESHOLD
            && math.abs(accelData.z) < ACCEL_THRESHOLD) {
            signal = 2;
        } else if (math.abs(accelData.x) < ACCEL_THRESHOLD
            && accelData.y < -ACCEL_THRESHOLD
...
function publishToChannel(accelData) {
